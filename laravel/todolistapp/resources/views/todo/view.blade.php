                ...
                <div class="card-body">
                    <div class="todo-details">
                        <div class="todo-section">
                            ...
                            <p class="todo-value">{{ $todo->name }}</p>
                        </div>
                        <hr class="todo-divider">
                        <div class="todo-section">
                            <h4 class="todo-detail">Description</h4>
                            <p class="todo-value">{{ $todo->description }}</p>
                        </div>
                        <hr class="todo-divider">
                        <div class="todo-section">
                            <h4 class="todok-detail">Date & Time</h4>
                            <p class="todo-value">{{ $todo->created_at }}</p>
                        </div>
                    </div>
                    <form id="deleteForm" action="{{ route('todo.destroy', $todo->id) }}" method="POST">
                        @csrf
                        @method('DELETE')
                        ...
                        <div class="modal-dialog modal-lg" role="document">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form id="editForm" class="edit-form" action="{{ route('todo.update', $todo->id) }}" method="POST">
                                        @csrf
                                        @method('PUT')
                                        <div class="mb-3">
                                            <label for="editName" class="form-label">Edit Task Name:</label>
                                            <input type="text" class="form-control" id="editName" name="editName" value="{{ $todo->name }}">
                                        </div>

                                        <div class="mb-3">
                                            <label for="editTitle" class="form-label">Edit Task Title:</label>
                                            <input type="text" class="form-control" id="editTitle" name="editTitle" value="{{ $todo->title }}">
                                        </div>

                                        <div class="mb-3">
                                            <label for="editDescription" class="form-label">Edit Task Description:</label>
                                            <textarea class="form-control" id="editDescription" name="editDescription" rows="4">{{ $todo->description }}</textarea>
                                        </div>
